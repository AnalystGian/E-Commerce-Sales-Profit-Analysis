...
SELECT 
	SUM("Sales") / COUNT(*) AS avg_transaction_value
FROM ecommerce_sales_data;



-- 4. How many transactions occur per day / per month?
SELECT *
FROM ecommerce_sales_data;

SELECT
    CAST("Order Date" AS DATE) AS order_day,
    COUNT(*) AS total_transactions
FROM ecommerce_sales_data
GROUP BY order_day
ORDER BY order_day;

-- Let's try to do it with monthly transactions
SELECT
	CAST(DATE_TRUNC('month', "Order Date") AS DATE) AS month,
	COUNT(*) AS total_transactions
FROM ecommerce_sales_data
...
-- 6. Which products sell the most by quantity?
SELECT
	"Product Name",
...
-- 8. What is the average profit margin per transaction?
SELECT
...
-- 9. Which products generate the most repeat sales entries?
SELECT
	"Product Name",
	COUNT(*) AS total_sales_entries
FROM ecommerce_sales_data
GROUP BY "Product Name"
ORDER BY total_sales_entries DESC
LIMIT 3;



-- 10. Are there any products that are selling but not profitable?
SELECT
	"Product Name",
	AVG("Profit" / "Sales") AS avg_profit_margin
FROM ecommerce_sales_data
GROUP BY "Product Name"
HAVING AVG("Profit" / NULLIF("Sales", 0)) <= 0
ORDER BY avg_profit_margin;


-- “How does profit compare to sales over time?”
-- Low-margin month detection (CTE)
-- 11. Which product categories are growing or declining month-over-month?
-- 12. What percentage of total revenue comes from the top 20% of products?
-- 13. Which products have high sales volume but low profit margins?
-- 14. Which products appear most frequently in transactions?
-- 15. What is the average time between transactions for the same product?
-- 16. Which combinations of category and region are most profitable?
-- 17. Which regions generate high revenue but low profit?
-- 18. What is the profit contribution of each category to total profit?
-- 19. How does quantity sold relate to profit margin?
-- 20. If the business had to drop 1 category, which one hurts profit the least?